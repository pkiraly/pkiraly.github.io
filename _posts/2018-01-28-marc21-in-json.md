...
This page displayes MARC21 structure in formatted JSON format. It is generated from the Java classes which describe the structure. There are some more attributes which is not exported into the JSON format, but it is a work in progress. If you miss something, please let me know via the [issue tracker](https://github.com/pkiraly/metadata-qa-marc/issues/45). Credits: the JSON schema was requested by [Jakob Voß](https://twitter.com/nichtich) and [Johann Rolschewski](https://twitter.com/johrols/).
...
```
Note: Jakob Voß published [Avream specification](http://format.gbv.de/schema/avram/specification) after the publication of theoriginal version of this post. The content the JSON format was chanced, and the current version valid against Avram.
